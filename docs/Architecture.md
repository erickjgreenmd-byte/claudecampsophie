...
| Child | Parent-issued single-use pairing code → API-issued child access token (short-lived, signed with an API-only key) + rotating opaque refresh token stored hashed | `pl_child` with API-set claims `{role:'pl_child', child_id, family_id, child_session_id}` | Not a Supabase auth user. The child token is not a Supabase JWT, so PostgREST rejects it; `pl_child` is never granted to the PostgREST authenticator. RLS re-checks that the child session is live. Refresh is idempotent on an optional client `refreshRequestId`: rotation records the id on the row it retires, and a re-presented rotated token is served only when it carries that id and its replacement is still unclaimed (BUG-244, migration 0880). Round 5 found that “unclaimed” is not “lost” (BUG-248), so the recovery is additionally bounded to two minutes from the rotation — ANDed with the id, never ORed — and every recovery writes an audit event so a replay is visible rather than silent. Every other re-presentation revokes the session, as before. |
