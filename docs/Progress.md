...
## Current state (2026-09-25; code at `c49a7e7`)
...
dissent is in Owner action #24 and Threat_Model T34). MCP connectors for Supabase, Stripe and Expo are attached
to the build session but no project, account or EAS project of PencilLift's exists yet (Connections).
...
Last local full gate (`scripts/verify.sh`, exit 0), run in an isolated worktree on the exact tree of `c49a7e7`:
api 876, domain 3,613, db 317, web 431, mobile 459, ai 49, contracts 15, ui-tokens 4 (5,764 tests, 0 failed,
0 skipped), gate audit, finance, release-artifact scan with negative control. CI runs and their results are listed in `docs/Test_Evidence.md` (the table
...
| Schema 0001–0780 with RLS, append-only ledgers, deletion purge, jobs, identity hardening, school-report zone pin, reward rules, safety screening (0760; its hold mechanism is unused since the owner decision), fake-catalog guard (0770), request instant for queued work (0780), parent flag review and email state (0790), Amazon Appstore channel (0800) | `supabase/tests` 255 tests on real Postgres 16 |
...
- Super-admin dashboard (company watch-list, support cases and refunds, subscriptions and revenue; branded,
  tested) — workflow `wf_f4a66218-84d`; data/API and mobile parts checked, web part in its check. The lead's
  stubs for it (`support.ts`, `admin-ops.ts` routes and contracts, `/app/support`, `/admin/support`,
  `/admin/revenue`, the domain `ops` subpath, the Support link) are in the working tree, uncommitted until the
  workflow's checkers and the full gate pass.

## Next exact steps

1. Integrate the super-admin dashboard once its checkers pass: run `scripts/verify.sh` on the combined tree,
   commit, push, confirm CI, record; then screenshot the branded screens and the dashboard for the owner.
2. Confirm CI green on the latest SHA; record it in `docs/Test_Evidence.md`.
3. Owner actions in `docs/Owner_Actions.md` (#1–#30) unblock everything else: accounts (Apple, Google Play, Amazon
