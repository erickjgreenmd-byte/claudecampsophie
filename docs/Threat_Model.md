...
| T33 | A child's severe-risk disclosure gets AI tutoring or nothing; model output takes a companion persona, asks for contact or secrecy | Deterministic first-layer screen before and after generation; severe → no AI call, reviewed template with resources, escalated system report without homework text; model output that screens severe falls back | `packages/domain/src/safety` corpus, `safety-screening.test.ts` (BUG-084) | Software-tested; English-first, provider moderation blocked, templates unapproved |
| T34 | An abuse-type flag reaches the household member it may concern | Abuse/sexual/secrecy system reports are held from the family (RLS on `family_visible`; audit rows carry no family id while held) until an owner releases them | `safety_screening.test.ts`, `safety-screening.test.ts` | Software-tested; the child's own answer and safety row stay readable to the family via the Data API (Owner action #25) |
