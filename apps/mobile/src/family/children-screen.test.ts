...
describe('a deletion-pending child on the Children screen (ACC-FAM-03)', () => {
  /**
   * GET /v1/family keeps a child whose data deletion is `requested`/`processing` in its list — the
   * privacy screens resolve the nickname out of that list for the pending-deletion list, the
   * child's export rows and any safety report about it, and the request is still cancellable — and
   * flags it with `deletionPending`. The API refuses pairing, activation and edits for such a child,
   * so this screen must say so and offer no control that would only dead-end.
   */
